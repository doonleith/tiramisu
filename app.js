...
document.querySelectorAll('.add').forEach(button=>button.onclick=()=>openForm(null,button.dataset.type));$('close').onclick=()=>$('dialog').close();$('transactions').onclick=event=>{const id=event.target.dataset.edit;if(id)openForm(transactions.find(item=>item.id===id))};$('sign-in').onclick=signIn;$('landing-sign-in').onclick=signIn;$('sign-out').onclick=()=>client?.auth.signOut();
