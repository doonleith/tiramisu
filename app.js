...
let month = new Date(), transactions = [], ledgers = [], activeLedgerId = null, client, user, misuMessages = [], misuDraft = null; month.setDate(1);
...
function render() {
  ...
  $('spent-detail').textContent = expenses.length ? `${expenses.length} expense ${expenses.length === 1 ? 'entry' : 'entries'}${largest ? ` · ${largest} highest` : ''}` : 'No expenses recorded yet';
  ...
  $('transactions').classList.toggle('is-scrollable', recent.length > 10); $('transactions').innerHTML = recent.length ? recent.map(item => { const [, colour, iconId] = categoryInfo(item.category, item.type), detail = item.recurringTransactionId ? `${item.category} · Monthly · ${ordinal(Number(item.date.slice(-2)))}` : `${item.category} · ${new Date(`${item.date}T12:00:00`).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}`; return `<div class="transaction"><span class="icon category-tile" style="background:${tileBackground(colour, iconId)}" aria-hidden="true">${iconSvg(iconId)}</span><button class="transaction-main" data-edit="${item.id}" aria-label="Open ${escapeHtml(item.note || item.category)} transaction"><b>${escapeHtml(item.note || item.category)}</b><small>${detail}</small></button><em class="${item.type === 'income' ? 'positive' : ''}">${item.type === 'income' ? '+' : '−'}${money(item.amount)}</em><button class="edit-button" data-edit="${item.id}" aria-label="Edit ${escapeHtml(item.note || item.category)}" title="Edit transaction">✎</button></div>` }).join('') : '<div class="empty">No transactions in this period yet.<br>Start by adding your first one.</div>';
  renderPeriod();
}
...
async function switchLedger(id, keepFocus = false) { if (id === activeLedgerId) return; activeLedgerId = id; misuDraft = null; misuMessages = []; localStorage.setItem('tiramisu-active-ledger', id); renderTabs(); if (keepFocus) $('tabs').querySelector(`[data-ledger="${id}"]`)?.focus(); await load() }
...
$('form').onsubmit = async event => {
  ...
  try {
    ...
    const result = id
      ...
      : await client.from('transactions').insert(item);
