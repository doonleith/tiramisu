...
function openForm(transaction,selectedType){
  if(!user)return signIn();const editing=Boolean(transaction),type=transaction?.type||selectedType||'expense';$('form').reset();$('id').value=transaction?.id||'';$('type').value=type;$('form-title').textContent=editing?'Edit transaction':`Add ${type}`;$('form-note').textContent=editing?'Update entry':`New ${type}`;fillCategories(type,transaction?.category);$('amount').value=transaction?.amount||'';$('date').value=transaction?.date||localDate();$('note').value=transaction?.note||'';$('repeat-monthly').checked=Boolean(transaction?.recurringTransactionId);$('delete').style.visibility=editing?'visible':'hidden';$('dialog').showModal();$('amount').focus()}

async function setUser(nextUser){user=nextUser;$('user').hidden=!user;$('landing').hidden=Boolean(user);$('app-view').hidden=!user;if(user){$('name').textContent=user.user_metadata.full_name||user.email;await load()}else{transactions=[];render()}}
async function signIn(){if(!client)return alert('The app is still loading. Please try again.');const{error}=await client.auth.signInWithOAuth({provider:'google',options:{redirectTo:`${location.origin}${location.pathname}?v=20`}});if(error)alert(`Google sign-in could not start: ${error.message}`)}
