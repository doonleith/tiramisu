...
document.querySelectorAll('.add').forEach(button=>button.onclick=()=>openForm(null,button.dataset.type));$('close').onclick=()=>$('dialog').close();$('close-tab').onclick=()=>$('tab-dialog').close();$('new-tab').onclick=()=>{$('tab-form').reset();$('tab-dialog').showModal();$('tab-name').focus()};$('tabs').onclick=event=>{const id=event.target.closest('[data-ledger]')?.dataset.ledger;if(id)switchLedger(id)};$('transactions').onclick=event=>{const id=event.target.dataset.edit;if(id)openForm(transactions.find(item=>item.id===id))};$('landing-sign-in').onclick=signIn;$('sign-out').onclick=()=>client?.auth.signOut();
