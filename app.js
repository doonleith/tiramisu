...
function renderPeriod(){$('period-label').textContent=periodName()}
...
function render(){
  ...
  const selectedMonthName=month.toLocaleDateString('en-GB',{month:'long'});
  $('income').textContent=money(income);$('spent').textContent=money(spent);$('balance').textContent=money(balance);$('income-detail').textContent=incomeEntries.length?`${incomeEntries.length} income ${incomeEntries.length===1?'entry':'entries'} for ${selectedMonthName}`:'No income recorded yet';
  ...
  $('spent-detail').textContent=expenses.length?`${expenses.length} expense ${expenses.length===1?'entry':'entries'} for ${selectedMonthName}`:'No expenses recorded yet';
  $('detail').textContent=income?`${money(spent)} spent from ${money(income)} income.`:'Add income and expenses to get started.';$('total').textContent=`${money(spent)} spent`;
