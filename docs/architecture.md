...
Browser
  ...
  └─ Supabase
       ...
       └─ Postgres: ledgers, ledger_members, ledger_invites,
                    transactions, recurring_transactions
            └─ RLS: signed-in users can access owned or joined spaces
```
...
- Creating one-time invite links and accepting shared-space invitations.
...
Links an authenticated user to a shared money space. Owners remain identified
by `ledgers.user_id`; this table stores additional members and their display
names.

### `ledger_invites`

Stores one-time invite tokens. An invite expires after seven days and records
who accepted it. Database functions create and accept invites without exposing
...
| `created_by_name` | Display attribution for shared-space activity |
...
- Allow owners and accepted members to view a money space and its entries.
- Allow members to create, update, and delete entries in joined spaces.
- Restrict invite creation, member removal, and space management to the owner.
- Require new entries to record the authenticated creator.
- Preserve creator attribution when another member edits an entry.
