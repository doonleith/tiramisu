...
const tools = [
  ...
  { type: "function", function: { name: "draft_transaction", description: "Prepare, but never save, a transaction after the user asks to add one. Ask a concise follow-up when an amount, type, or category is missing.", parameters: { type: "object", properties: { type: { type: "string", enum: ["income", "expense"] }, amount: { type: "number", minimum: 0.01 }, category: { type: "string" }, date: { type: "string", description: "Date in YYYY-MM-DD format." }, note: { type: "string" }, repeat_monthly: { type: "boolean" }, payment_day: { type: "integer", minimum: 1, maximum: 31 } }, required: ["type", "amount", "category", "date", "repeat_monthly"], additionalProperties: false } } },
];
...
function safeDraft(value: Record<string, unknown>): Draft | null {
  ...
  const category = typeof value.category === "string" && type && categories[type].includes(value.category) ? value.category : null;
  const date = typeof value.date === "string" && datePattern.test(value.date) ? value.date : null;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const system = `You are Misu, a calm and concise personal-finance assistant for the money space “${ledger.name}”. Today's date is ${new Date().toISOString().slice(0, 10)}. You only know information returned by tools; never invent amounts or transactions. Use UK pounds and plain language. Use a read tool for every factual money question. When asked to add a transaction, collect missing details, then call draft_transaction. Never claim a draft is saved; tell the user it is ready to review and confirm. Do not offer financial, tax, credit, or investment advice.`;
